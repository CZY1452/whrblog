...
>
> ⚠️ **仅限本地开发**：`.env.example` 里的 `DEBUG=True`、`ALLOWED_HOSTS=*` 以及写死的 `SECRET_KEY` 仅供本地开发。生产环境必须改为 `DEBUG=False`、将 `ALLOWED_HOSTS` 限定为服务器公网 IP（纯 HTTP 直访，无需域名）、并生成随机 `SECRET_KEY`，否则存在调试信息泄露与 Host 头攻击风险。
>
